...
function stub(f: Function): SinonStub {
  return f as SinonStub;
}
...
describe('MysqlDatabase', () => {
  ...
  describe('getTableTypes', () => {
    ...
    it('gets custom types from enums', async () => {
      stub(MysqlDBReflection.prototype.getEnumTypes).returns(
        Promise.resolve({
          enum1: [],
          enum2: [],
        }),
      );
      stub(MysqlDBReflection.prototype.getTableDefinition).returns(Promise.resolve({}));
    ...
    it('gets table definitions', async () => {
      stub(MysqlDBReflection.prototype.getEnumTypes).returns(Promise.resolve({}));
      stub(MysqlDBReflection.prototype.getTableDefinition).returns(
          Promise.resolve({
            table: {
              udtName: 'name',
              nullable: false,
      ...
      await db.getTableTypes('tableName', 'tableSchema', options);
      assert.deepEqual(
        stub(MysqlDBReflection.prototype.getTableDefinition).getCall(0).args,
