...
export class MysqlDatabase implements Database {
  ...
  public query(queryString: string): Promise<Record<string, any>[]> {
    return this.queryAsync(queryString);
  }

  public async getEnumTypes(schema?: string): Promise<any> {
    const enums: any = {};
  ...
  public async getTableTypes(tableName: string, tableSchema: string, options: Options): Promise<TableDefinition> {
  ...
  public async getSchemaTables(schemaName: string): Promise<string[]> {
    const schemaTables = await this.queryAsync(
      'SELECT table_name ' + 'FROM information_schema.columns ' + 'WHERE table_schema = ? ' + 'GROUP BY table_name',
      [schemaName],
    );
    return schemaTables.map((schemaItem: { table_name: string }) => schemaItem.table_name);
  }

  public queryAsync(queryString: string, escapedValues?: Array<string>): Promise<Record<string, any>[]> {
    return new Promise((resolve, reject) => {
      this.db.query(queryString, escapedValues, (err: MysqlError | null, results: any | undefined) => {
        if (err) {
          return reject(err);
        }
